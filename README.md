...
Для проверки работоспособности бота, необходимо перейти по ссылке https://max.ru/t591_hakaton_bot или найти его по публичному имени  `@t591_hakaton_bot`. Для демонстрации всех ролей и их функций была добавлена роль супер пользователя. Это роль по умолчанию проставляется всем новым пользователям (нужно просто запустить бота). Вам будет предложено выбрать роль: администратор, преподаватель или студент. Под каждой этой ролью Вы можете проверить все функции и сценарии работы. На каждом этапе работы Вы можете сменить роль, нажав на кнопку "К выбору роли".
Так как Вы не являетесь настоящим студентом или преподавателем, Вам будет доступно все расписание, предметы, группы и показаны все оценки и вся посещаемость (вместе со статистикой).
Данный режим доступен только из ветки `super-user` и предоставляется исключетельно для проверки решения.
